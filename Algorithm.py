...
def match(time_dict, site_dict, total_people):
    accept_time = filter_proportion(time_dict, time_proportion, total_people)  # 可接受的時間
    if len(accept_time) == 0: return None
    if debug: print('accept_time:', accept_time)
    optimal_time = find_optimal(accept_time)  # 找出最佳的時間，並且判斷是否有複數個
